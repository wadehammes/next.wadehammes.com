...
- **Examples**: [`SpiralsConfig.factory.ts`](../../src/tests/factories/SpiralsConfig.factory.ts) (Spirals), [`RichText.factory.ts`](../../src/tests/factories/RichText.factory.ts), [`HomeDocument.factory.ts`](../../src/tests/factories/HomeDocument.factory.ts), and [`LinksDocument.factory.ts`](../../src/tests/factories/LinksDocument.factory.ts) (Prismic). `HomeDocumentFactory.buildParsedPage()` / `LinksDocumentFactory.buildParsedPage()` return parsed CMS shapes via [`parseHomeDocument`](../../src/prismic/parsePage.ts) / [`parseLinksDocument`](../../src/prismic/parseLinks.ts)—use them for component specs that take parsed data.
- **Prismic document factories**: accept partial top-level fields and partial nested `data` overrides (see `LinksDocumentBuildAttributes` in [`LinksDocument.factory.ts`](../../src/tests/factories/LinksDocument.factory.ts)) so specs can pin one CMS field without redefining the whole document.
...
- **External npm packages**: manual mock under **[`__mocks__/`](../../__mocks__/)** (for node modules) or shared helpers under **`src/tests/mocks/`**, then opt-in with **`jest.mock("package-name")`** in the PO—see [`__mocks__/@prismicio/react.tsx`](../../__mocks__/@prismicio/react.tsx), [`__mocks__/next/image.tsx`](../../__mocks__/next/image.tsx) (Links POs), and [`prismicReactMock.tsx`](../../src/tests/mocks/prismicReactMock.tsx). Do **not** use `jest.requireActual("@prismicio/react")` in mock factories (ESM; Jest cannot load it). Avoid unit-testing [`getPage.ts`](../../src/prismic/getPage.ts) directly—it pulls `@prismicio/next` and Next server modules; cover getters indirectly via parser specs + `HomePage` integration specs.
